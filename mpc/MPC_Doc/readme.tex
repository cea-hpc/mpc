...
% MAIN HEADER %
%%%%%%%%%%%%%%%
\title{MPC\\Multi-Processor Communications Library\\Version {\MPCVERSION} User's Guide}

\author{Patrick Carribault \and Marc P\'{e}rache}
\author{http://www.paratools.fr/mpc}
...
may execute the \texttt{mpc\_cc} compiler for GNU:
...
\end{enumerate}
To use the Intel backend, please use the scripts \texttt{mpc\_icc}, 
\texttt{mpc\_icpc}, and \texttt{mpc\_ifort} to compile the application. 
...
\subsection*{Q - Why is my MPI program crashing when I use the MPC compilers? }
...
\begin{enumerate}
\item Use the compiler option \texttt{-fmpc-privatize} to automatically privatize the global variables. In this mode, every global variable is duplicated for every MPI task such as the code can run correctly with MPC. Note than in versions older than 2.4.0, the application has to be compiled as a dynamic library for this solution to work.
\item Use the option -Wmpc with the patched GCC compiler to generate warnings. In this mode, the compiler will warn you about every global variable declared in the program.
\end{enumerate}
...
\subsection*{Q - Why won't HDF5 compile with MPC?}
...
\begin{itemize}
  ...
  \item ParaTools Staff $\quad$ info@paratools.fr
\end{itemize}
