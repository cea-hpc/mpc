...
void mpcomp_GOMP_offload_unregister_ver (unsigned version, const void *host_table,
			     int target_type, const void *target_data)
{
   sctk_nodebug("[Redirect GOMP]%s:\tBegin",__func__);
   not_implemented();
...
void mpcomp_GOMP_target_enter_exit_data (int device, size_t mapnum, void **hostaddrs,
			     size_t *sizes, unsigned short *kinds,
			     unsigned int flags, void **depend)
{
   sctk_nodebug("[Redirect GOMP]%s:\tBegin",__func__);
