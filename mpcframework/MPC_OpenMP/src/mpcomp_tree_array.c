...
static int 
__mpcomp_tree_array_get_stage_nodes_num_per_depth( const int* shape, __UNUSED__ const int max_depth, const int depth )
{
    int i, count;
...
__mpcomp_openmp_node_initialisation( mpcomp_meta_tree_node_t* root, const int* tree_shape, const int max_depth, const int rank )
{
    ...
    me->first_child_array = (int*) __mpcomp_tree_array_get_first_child_array( tree_shape, max_depth, rank );
...
__mpcomp_openmp_mvp_initialisation( void* args )
{
    ...
    me->fathers_array = (int*)__mpcomp_tree_array_get_father_array_by_depth( tree_shape, max_depth, rank );
...
__mpcomp_alloc_openmp_tree_struct( int* shape, int max_depth, const int* cpus_order, const int place_depth, const int place_size, const mpcomp_local_icv_t icvs )
{
    ...
    root = ( mpcomp_node_t* ) mpcomp_alloc( sizeof( mpcomp_node_t ) );
    sctk_assert( root ); 
    ...
    for( i = 1; i < leaf_n_num; i++ )
    {
        ...
        const int pu_id = ( mpc_common_topo_get_current_cpu() + target_vp ) % mpc_common_topo_get_pu_count(); 
        ...
        ret = sctk_user_thread_create( &(threads[i-1]), &__attr,__mpcomp_openmp_mvp_initialisation, &( args[i] ) );
        sctk_assert( !ret );
