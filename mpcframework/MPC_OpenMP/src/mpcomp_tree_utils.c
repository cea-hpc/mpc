...
int mpcomp_is_leaf( mpcomp_instance_t* instance, int globalRank )
{
	...
	sctk_assert( globalRank >= 0 && MPCOMP_TASK_INSTANCE_GET_ARRAY_TREE_TOTAL_SIZE( instance ) );
	return ( globalRank >= ( MPCOMP_TASK_INSTANCE_GET_ARRAY_TREE_TOTAL_SIZE( instance ) - instance->nb_mvps ) );
}
...
mpcomp_get_neighbour(int globalRank, int i)
{
   ...
   int* treeBase = thread->instance->tree_base;
	...
	{
		mpcomp_mvp_t *mvp = (mpcomp_mvp_t *) ( MPCOMP_TASK_MVP_GET_TREE_ARRAY_NODE( thread->mvp, globalRank) );
		path = MPCOMP_TASK_MVP_GET_PATH( mvp );
	  	vectorSize = mvp->father->depth + 1;
	} 
	else 
	{
	  	mpcomp_node_t *node = (mpcomp_node_t *) ( MPCOMP_TASK_NODE_GET_TREE_ARRAY_NODE( thread->mvp, globalRank) );;
   ...
   int nbSubleaves = 1;
	...
	for ( j = 0; j < vectorSize; j++ ) 
	{
		int base = treeBase[vectorSize-1-j]; 
	  	int level_size = MPCOMP_TASK_INSTANCE_GET_ARRAY_TREE_LEVEL_SIZE( thread->instance, j );

	  	sctk_assert( vectorSize - 1 - j >= 0 && vectorSize -1 -j < thread->mvp->father->depth + 1); 
   ...
   for( i = 0; i < depth; i++) 
	{
		ancestor_id += path[depth-1-i] * nbSubLeaves;
	  	...
	  	firstRank += MPCOMP_TASK_INSTANCE_GET_ARRAY_TREE_LEVEL_SIZE( thread->instance, i );
	}
...
static void 
__mpcomp_task_check_neighbourhood_r(mpcomp_node_t* node)
{
   int i, j;
   ...
   for( j = 1; j < MPCOMP_TASK_INSTANCE_GET_ARRAY_TREE_LEVEL_SIZE( thread->instance, node->depth ); j++)
	{
   	...
   	case MPCOMP_CHILDREN_NODE:
     	...
     	case MPCOMP_CHILDREN_LEAF:
	  		for( i = 0; i < node->nb_children; i++) 
			{
	      	...
	      	for( j = 1; j < MPCOMP_TASK_INSTANCE_GET_ARRAY_TREE_LEVEL_SIZE( thread->instance, node->depth + 1 ); j++) 
				{
