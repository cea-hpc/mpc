...
/* INLINE FUNCTIONS */
static inline kmp_int32 mpcomp_intel_compute_trip_count( kmp_int32 lb, kmp_int32 b, kmp_int32 incr )
{
    kmp_int32 ret = 0;
...
static inline long __kmpc_for_static_init_no_chunk_long( long lb, long b, long incr, long *from, long *to) 
{
    const kmp_int32 trip_count = mpcomp_intel_compute_trip_count( lb, b, incr);
    const long chunk_num  = __mpcomp_static_schedule_get_single_chunk( lb, b, incr, from, to);  
...
static inline long __kmpc_for_static_islastiter( long rank, long thread_num, long chunk_num, long old_to, long new_from, long new_to, long incr )
{
    long isLastIter = 0;
    ...
    {
        isLastIter = (incr > 0) ? new_from <= old_to && new_to > old_to - incr : isLastIter; 
        isLastIter = (incr < 0) ? new_from >= old_to && new_to < old_to - incr : isLastIter; 
    }
