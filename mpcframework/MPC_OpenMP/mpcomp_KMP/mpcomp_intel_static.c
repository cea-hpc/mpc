...
void __kmpc_for_static_init_4( ident_t *loc, kmp_int32 gtid, kmp_int32 schedtype, kmp_int32 * plastiter, kmp_int32 * plower, kmp_int32 * pupper, kmp_int32 * pstride, kmp_int32 incr, kmp_int32 chunk ) 
{
    ...
    switch( schedtype ) 
    {
        case kmp_sch_static:
        {
            chunk_num = __kmpc_for_static_init_no_chunk_long( *plower, *pupper+incr, incr, &from, &to );
            if( plastiter != NULL )
            {
                *plastiter = __kmpc_for_static_islastiter( t->rank, t->info.num_threads, chunk_num, (long) *pupper, from, to, incr); 
            }
            *pupper = (kmp_int32) to; 
    ...
    kmp_int32 * pstride, kmp_int32 incr, kmp_int32 chunk ) 
{
    ...
    __mpcomp_loop_gen_infos_init_ull( loop_infos, (unsigned long long) *plower, (unsigned long long)*pupper+(unsigned long long)incr, (unsigned long long)incr, (unsigned long long) chunk, (incr > 0) );
    ...
    switch( schedtype ) 
    {
        case kmp_sch_static:
            ...
            res =  __mpcomp_static_schedule_get_single_chunk_ull( &(loop_infos->loop.mpcomp_ull), incr, &from, &to ) ;
    ...
    kmp_int64 * pstride, kmp_int64 incr, kmp_int64 chunk ) 
{
  ...
  sctk_nodebug( "__kmpc_for_static_init_8u: siweof long = %d, sizeof long long %d",
    ...
    switch( schedtype ) 
    {
        case kmp_sch_static:
            /* Get the single chunk for the current thread */
            res =  __mpcomp_static_schedule_get_single_chunk_ull( &(loop_infos->loop.mpcomp_ull), &from, &to ) ;
