...
mpcomp_task_tree_infos_alloc( const int tree_array_size )
{
    ...
    task_tree_infos->tree_array = mpcomp_malloc( 0, sizeof( mpcomp_node_ptr_t* ) * numa_node_number, 0 );
    sctk_assert( task_tree_infos->tree_array );

    sctk_assert( tree_array_size > 0 );
	for( i = 0; i < numa_node_number; i++)
    {
        task_tree_infos->tree_array[i] = mpcomp_malloc( 1, sizeof( mpcomp_node_ptr_t ) * tree_array_size, i );
        sctk_assert( task_tree_infos->tree_array[i] );
		memset( task_tree_infos->tree_array[i], 0, sizeof( mpcomp_node_ptr_t ) * tree_array_size );
    }
	
	task_tree_infos->tasklistNodeRank = mpcomp_malloc( 1, sizeof( int ) * MPCOMP_TASK_TYPE_COUNT, 0 );
	sctk_assert( task_tree_infos->tasklistNodeRank );
	memset( task_tree_infos->tasklistNodeRank, 0, MPCOMP_TASK_TYPE_COUNT * sizeof( int ) );
...
static inline mpcomp_task_tree_infos_t*
mpcomp_task_tree_infos_duplicate( mpcomp_task_tree_infos_t* parent_task_tree_infos, int id_numa )
{
	...
	new_task_tree_infos = mpcomp_malloc( 1, sizeof( mpcomp_task_tree_infos_t ), id_numa );
	sctk_assert( new_task_tree_infos );	

	memcpy( new_task_tree_infos, parent_task_tree_infos, sizeof( mpcomp_task_tree_infos_t ) ); 
...
static inline void
mpcomp_task_tree_register_node_in_all_numa_node_array( mpcomp_node_ptr_t** tree_array, mpcomp_node_t* node, const int global_rank )
{
	int i;
