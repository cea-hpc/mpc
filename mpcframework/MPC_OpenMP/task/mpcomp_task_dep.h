...
#define MPCOMP_TASK_DEP_LOCK_NODE( node ) \
	sctk_spinlock_lock( &( node->lock ) )

#define MPCOMP_TASK_DEP_UNLOCK_NODE( node ) \
...
static char* 
mpcomp_task_dep_type_to_string[ MPCOMP_TASK_DEP_COUNT ] = { 
...
mpcomp_task_dep_htable_op_to_string[ MPCOMP_TASK_DEP_HTABLE_OP_COUNT ] = { 
	"MPCOMP_TASK_DEP_HTABLE_OP_INSERT", 
	"MPCOMP_TASK_DEP_HTABLE_OP_DELETE", 
...
mpcomp_task_dep_alloc_task_htable( mpcomp_task_dep_hash_func_t hfunc )
{
   ...
   memset( new_htable, 0, infos_size + array_size );
	...
	new_htable->buckets = ( mpcomp_task_dep_ht_bucket_t* ) ( ( uintptr_t ) new_htable + infos_size ); 
...
static inline mpcomp_task_dep_ht_entry_t* 
mpcomp_task_dep_ht_find_entry( mpcomp_task_dep_ht_table_t* htable, uintptr_t addr )
{
