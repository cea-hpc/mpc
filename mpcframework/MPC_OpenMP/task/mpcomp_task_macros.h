...
#define MPCOMP_TASK_STATUS_IS_INITIALIZED( status )	\
	(  sctk_atomics_load_int( &( status ) )				\
		== MPCOMP_TASK_INIT_STATUS_INITIALIZED )
		
#define MPCOMP_TASK_STATUS_TRY_INIT( status )			\
	(  sctk_atomics_cas_int( &( status ),					\
			MPCOMP_TASK_INIT_STATUS_UNINITIALIZED,			\
			MPCOMP_TASK_INIT_STATUS_INIT_IN_PROCESS )		\
		 == MPCOMP_TASK_INIT_STATUS_UNINITIALIZED )		  

#define MPCOMP_TASK_STATUS_CMPL_INIT( status )			\
	(	sctk_atomics_store_int( &( status ),				\
			MPCOMP_TASK_INIT_STATUS_INITIALIZED ) ) 
...
#define MPCOMP_TASK_MVP_SET_TREE_ARRAY_RANK( mvp, id ) do{ mvp->task_infos.tree_array_rank = id; }while(0)

#define MPCOMP_TASK_NODE_GET_TREE_ARRAY_RANK( node ) node->task_infos.tree_array_rank;
#define MPCOMP_TASK_NODE_SET_TREE_ARRAY_RANK( node, id ) do{ node->task_infos.tree_array_rank = id; }while(0)
...
#define MPCOMP_TASK_MVP_SET_PATH( mvp, ptr ) do{ mvp->task_infos.path = ptr; }while(0);

#define MPCOMP_TASK_NODE_GET_PATH( node ) node->task_infos.path
#define MPCOMP_TASK_NODE_SET_PATH( node, ptr ) do{ node->task_infos.path = ptr; }while(0);

#define MPCOMP_TASK_MVP_GET_TASK_LIST_NODE_RANK( mvp, type ) mvp->task_infos.tasklistNodeRank[type]
#define MPCOMP_TASK_MVP_SET_TASK_LIST_NODE_RANK( mvp, type, id ) do{ mvp->task_infos.tasklistNodeRank[type] = id; }while(0)

#define MPCOMP_TASK_MVP_GET_TASK_LIST_HEAD( mvp, type ) mvp->task_infos.tasklist[type] 
#define MPCOMP_TASK_MVP_SET_TASK_LIST_HEAD( mvp, type, ptr ) do{ mvp->task_infos.tasklist[type] = ptr; } while(0) 

#define MPCOMP_TASK_NODE_GET_TASK_LIST_HEAD( node, type ) node->task_infos.tasklist[type] 
#define MPCOMP_TASK_NODE_SET_TASK_LIST_HEAD( node, type, ptr ) do{ node->task_infos.tasklist[type] = ptr; } while(0) 

#define MPCOMP_TASK_MVP_GET_LAST_STOLEN_TASK_LIST( mvp, type ) mvp->task_infos.lastStolen_tasklist[type]
#define MPCOMP_TASK_MVP_SET_LAST_STOLEN_TASK_LIST( mvp, type, ptr ) do{ mvp->task_infos.lastStolen_tasklist[type] = ptr; }while(0)

#define MPCOMP_TASK_NODE_GET_TASK_LIST_RANK_BUFFER( node ) node->task_infos.tasklist_randBuffer 
#define MPCOMP_TASK_NODE_SET_TASK_LIST_RANK_BUFFER( node, ptr ) do{ node->task_infos.tasklist_randBuffer = ptr; }while(0)

#define MPCOMP_TASK_MVP_GET_TASK_LIST_RANK_BUFFER( mvp, rank ) \
	MPCOMP_TASK_NODE_GET_TASK_LIST_RANK_BUFFER( MPCOMP_TASK_MVP_GET_TREE_ARRAY_NODE( mvp, rank ) )

#define MPCOMP_TASK_MVP_SET_TASK_LIST_RANK_BUFFER( mvp, ptr ) do{ mvp->task_infos.tasklist_randBuffer = ptr; }while(0);
