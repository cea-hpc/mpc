...
static int
mpcomp_task_tree_task_list_alloc( mpcomp_tree_meta_elt_t* meta_elt, int tasklist_depth, int* tasklistNodeRank, mpcomp_tasklist_type_t type, const int depth, const int id_numa)
{
...
static void mpcomp_task_tree_init_task_tree_infos_leaf( mpcomp_mvp_t* mvp, mpcomp_task_tree_infos_t* task_tree_infos, const int rank, const int depth, const int id_numa ) 
{
   ...
   /* Retrieve the current thread information */
	...
	sctk_assert( rank < MPCOMP_TASK_INSTANCE_GET_ARRAY_TREE_TOTAL_SIZE( omp_thread_tls->instance ) );
	...
	mpcomp_task_tree_allocate_all_task_list( &meta_elt, omp_thread_tls->instance->team, task_tree_infos, rank, depth, id_numa ); 
}
...
mpcomp_task_tree_init_task_tree_infos_node( mpcomp_node_t* node, mpcomp_task_tree_infos_t* parent_task_tree_infos, int index ) 
{
	...
	sctk_assert( cur_globalRank < MPCOMP_TASK_INSTANCE_GET_ARRAY_TREE_TOTAL_SIZE( thread->instance ) );
	...
	child_task_tree_infos = mpcomp_task_tree_infos_duplicate( parent_task_tree_infos, node->id_numa );
	child_task_tree_infos->stage_size 	= parent_task_tree_infos->stage_size * tree_base_value;	
	...
	mpcomp_task_tree_allocate_all_task_list( &meta_elt, thread->instance->team, child_task_tree_infos, cur_globalRank, node->depth, node->id_numa ); 
...
void 
mpcomp_task_tree_infos_init( void )
{
	mpcomp_team_t* team;
	mpcomp_thread_t* omp_thread_tls;
		...
		if( !MPCOMP_TASK_TEAM_IS_INITIALIZED( team ) ) 
     	{
        	...
        	{ 
				...
				const int tree_array_size = MPCOMP_TASK_INSTANCE_GET_ARRAY_TREE_TOTAL_SIZE( omp_thread_tls->instance ); 

                sctk_assert( tree_array_size > 0 );
				task_tree_infos = mpcomp_task_tree_infos_alloc( tree_array_size );	

				sctk_assert( omp_thread_tls->mvp );
				mpcomp_task_tree_infos_init_r( omp_thread_tls->mvp->root, task_tree_infos, 0 );
...
void 
__mpcomp_task_list_infos_exit( void )
{
	mpcomp_thread_t* omp_thread_tls;
