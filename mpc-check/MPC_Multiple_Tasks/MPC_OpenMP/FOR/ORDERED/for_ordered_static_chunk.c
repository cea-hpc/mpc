/**
  Check a for loop with static schedule and ordered clause.
  */
#include <omp.h>
#include <omp_abi.h>
...
void * omp_f( void * shared ) {
  ...
  if ( __mpcomp_ordered_static_loop_begin( lb, b, step, chunk_size, &from, &to ) ) {
    do {
	...
	*count = *count + 1 ;
    ...
    } while ( __mpcomp_ordered_static_loop_next( &from, &to ) ) ;
  }
