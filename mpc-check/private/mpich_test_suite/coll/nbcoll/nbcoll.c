...
int main( int argc, char *argv[] )
{
	...
	MPI_Irecv( MPI_BOTTOM, 0, MPI_INT, 0, 0, MPI_COMM_WORLD, &r );
	t0 = MPI_Wtime();
	while (MPI_Wtime() - t0 < 3.0) sched_yield();
	fprintf(stderr,"MPI_Irecv from 0 should be done\n");
