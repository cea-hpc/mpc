...
SenderTest1( void )
{
	...
	for (j = 0; j < maxbufferlen; j += 500) {
	    ...
	    MPI_Isend(bufferspace[i], j, BasicTypes[i], dest, 
		      2000, MPI_COMM_WORLD, 
		      &(requests[act_send++]));
/* 	    MPI_Wait(&(requests[act_send-1]),statuses); */
...
ReceiverTest1( void )
{
	...
	for (j = 0; j < maxbufferlen; j += 500) {
	    /* Skip null datatypes */
	    if (BasicTypes[i] == MPI_DATATYPE_NULL) continue;
	    assert(MPI_Irecv(bufferspace[i], j, BasicTypes[i], src, 
			     2000, MPI_COMM_WORLD, &Req) == MPI_SUCCESS);
	    sprintf(message, "Send-Receive Test, Type %d, Count %d",
		    i, j);
	    assert(MPI_Wait(&Req, &Stat) == MPI_SUCCESS);
/*  	    MPI_Barrier(MPI_COMM_WORLD);  */
