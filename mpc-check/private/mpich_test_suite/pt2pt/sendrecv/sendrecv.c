...
#define src privatise[__mpc_get_rank()].src
#define dest privatise[__mpc_get_rank()].dest
#define do_test1 privatise[__mpc_get_rank()].do_test1
#define do_test2 privatise[__mpc_get_rank()].do_test2
#define do_test3 privatise[__mpc_get_rank()].do_test3

#define ntypes privatise[__mpc_get_rank()].ntypes
#define nolongdouble privatise[__mpc_get_rank()].nolongdouble
#define BasicTypes privatise[__mpc_get_rank()].BasicTypes
#define BasicNames privatise[__mpc_get_rank()].BasicNames


static const int verbose = 0;
static const int maxbufferlen = 10000;
...
ReceiverTest1()
{
	...
	if (Stat.MPI_SOURCE != src) {
	  fprintf(stderr, "*** Incorrect Source returned. tst1 %d != %d ***\n",
		  Stat.MPI_SOURCE, src);
	    Test_Failed(message);
	    passed = 0;
	...
	for (j = 0; j < maxbufferlen; j += 500) {
	    ...
	    sprintf(message, "Send-Receive Test, Type %d, Count %d",
		    i, j);
	    if (Stat.MPI_SOURCE != src) {
		fprintf(stderr, "*** Incorrect Source returned. test1 part 2***\n");
		Test_Failed(message);
		passed = 0;
	    } else if (Stat.MPI_TAG != 2000) {	
		fprintf(stderr, "*** Incorrect Tag returned. ***\n");	    
...
ReceiverTest2()
{
	...
	sprintf(message, "Tag Selectivity Test, Tag %d",
		i);
	if (Stat.MPI_SOURCE != src) {
	    fprintf(stderr, "*** Incorrect Source returned. test2 ***\n");
	    Test_Failed(message);
...
count argument, datatype argument, tag, rank, buffer send and buffer recv\n" );
	...
	if (MPI_Send(buffer, 20, MPI_INT, dest, 
	    ...
	    Test_Failed("Invalid BIG Tag Test");
	    }
	else
	    Test_Passed("Invalid Tag Test");
    }
